...
export default function AwardWinning() {
  ...
  return (
    <section className="py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-8 transition-colors duration-300 bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12 sm:mb-16 lg:mb-20">
          ...
          >
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-emerald-600">Award-Winning</span>{" "}
            <span className="text-gray-900 dark:text-white">Digital Innovation</span>
          </motion.h1>
          <motion.p
            className="text-base sm:text-lg lg:text-xl max-w-3xl mx-auto leading-relaxed transition-colors duration-300 text-gray-600 dark:text-gray-300"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative w-full flex items-center justify-center">
          ...
          {!isMobile && (
            <>
              <button 
                onClick={goPrev}
                className="absolute left-4 z-10 p-2 rounded-full bg-white shadow-md hover:bg-gray-100 transition-colors"
              ...
              <button 
                onClick={goNext}
                className="absolute right-4 z-10 p-2 rounded-full bg-white shadow-md hover:bg-gray-100 transition-colors"
          ...
          <div className={`relative ${isMobile ? 'h-[400px]' : 'h-[450px] lg:h-[500px]'} w-full flex items-center justify-center overflow-hidden`}>
            {awards.map((award, originalIndex) => {
              ...
              return (
                ...
                >
                  {/* Card */}
                  <div className={`rounded-2xl shadow-xl overflow-hidden flex flex-col h-full border-2 transition-all duration-300 bg-white border-gray-100 hover:shadow-2xl hover:border-green-500/20`}>
                    {/* Image */}
                    ...
                    <div className="p-6 sm:p-7 lg:p-8 flex flex-col flex-grow text-center">
                      <h3 className="text-lg sm:text-xl lg:text-2xl font-bold mb-3 sm:mb-4 leading-tight transition-colors duration-300 text-gray-800">
                        {award.title}
                      </h3>
                      <p className="text-sm sm:text-base leading-relaxed flex-grow mb-4 sm:mb-6 transition-colors duration-300 text-gray-600">
                        {award.description}
                      </p>
                      <div className="inline-flex items-center justify-center px-4 py-2 rounded-full text-xs sm:text-sm font-semibold transition-colors duration-300 bg-green-50 text-green-700 border border-green-200">
                        {award.year}
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex justify-center mt-8 space-x-2">
            {awards.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-colors ${displayOrder[0] === index ? 'bg-green-500' : 'bg-gray-300'}`}
