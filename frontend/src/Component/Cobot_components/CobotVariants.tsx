...
function RobotShowcase() {
  ...
  const displayName =
    selectedCobot?.name ??
    translateWithKeyVariants('go2SpecName', currentSpec.id, currentSpec.name, [currentSpec.name]);
  const displayDescription =
    selectedCobot?.description ??
    translateWithKeyVariants('go2SpecDesc', currentSpec.id, currentSpec.description, [
      currentSpec.name,
    ]);
              ...
              {filteredCobots.length === 0 && (
                <div className="text-center py-12">
                  <div className="text-gray-400 dark:text-gray-500 text-lg">
                    {translateWithFallback('go2SearchNoResults', 'No cobots found matching your search.')}
                  </div>
                  <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">
                    {translateWithFallback('go2SearchNoResultsHelper', 'Try different keywords or browse all cobots.')}
                  </p>
                </div>
              )}
            </div>
...
<div className="w-full max-w-4xl mx-auto mb-10 relative">
  <Select value={selectedVariant} onValueChange={setSelectedVariant}>
    ...
    >
      <SelectValue
        placeholder={translateWithFallback('go2VariantDropdownPlaceholder', 'Choose your preferred variants')}
    ...
    {isDropdownOpen && (
      <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-black border-2 border-black dark:border-gray-400 rounded-xl shadow-xl z-50 max-h-56 overflow-y-auto">
        {robotSpecs.map((spec) => (
          ...
          >
            {translateWithKeyVariants('go2SpecName', spec.id, spec.name, [spec.name])}
          </div>
        ))}
      </div>
    )}
    ...
    {translateWithFallback('orderNow', 'Order Now')}
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">
                {translateWithFallback('go2GalleryHeading', 'Gallery')}
              </h3>
                ...
                return (
                  ...
                  >
                    <div className="text-left max-w-[120px]">
                      ...
                      >
                        {translateWithKeyVariants('go2FeatureLabel', feature.label, feature.label, [currentSpec.id])}
                      </div>
                      <div 
                      ...
                      >
                        {translateWithKeyVariants('go2FeatureDetail', feature.detail, feature.detail, [
                          feature.label,
                          currentSpec.id,
                        ])}
                      </div>
